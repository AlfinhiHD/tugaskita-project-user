...
const FormTugas = () => {
  ...
  const onSubmit = async (data: SubmitAjukanTugasFormValues) => {
    ...
    try {
      const response = await instance.post("/user-task/religion-req", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });

      if (response.data.message === "succes upload religion task request") {
        setIsSubmitting(false);
        Swal.fire("Success", "Berhasil submit ajukan tugas", "success");
        router.push("/riwayat-keagamaan");
      } else {
        setIsSubmitting(false);
